...
+   const newPost = { id, content, image: null, createdAt };
-   setPost(post);
...
+     reader.onload = async () => {
+       const iamgeData = reader.result;  // 内部的には base64 形式
+       const id = await db.posts.add({
+         text,
+         image: imageData,
+         createdAt,
+       });
+
+       const newPost = { id, text, image: imageData, createdAt };
+       setPosts((prev) => [newPost, ...prev]);
+     }
...
+ } else {
    ...
    const newPost = { id, text, image: null, createdAt };
    setPosts((prev) => [newPost, ...prev]);
-   setText('');
-   setImage(null);
+ }
