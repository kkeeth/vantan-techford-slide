...
<img src="https://placehold.jp/150x150.png" alt="テストフロー" class="h-60 mx-auto" />
...
テストにはいくつかの種類があります（これら以外にもたくさんあります）:
...
<img src="https://placehold.jp/150x150.png" alt="テストピラミッド" class="h-50 mx-auto" />
...
その中でも特によく使うのは `toBe, not.toBe, toThrow` になると思います．
...
describe('validateMessage', () => {
  ...
  it('141文字以上はエラー', () => {
    ...
    expect(validateMessage(text)).toBe('140 文字以内で入力してください');
  });
...
```ts
// ❌ 140, 141 がハードコードされている
it('141文字以上はエラー', () => {
  const text = 'a'.repeat(141);  // なぜ 141?
  expect(validateMessage(text)).toContain('140');
});
```

- `141` の意味がコードから読み取れない
- 仕様変更時にテストと実装の両方を修正する必要がある
- 不整合が起きやすい
...
**→ テスタブルなコードにリファクタリングしましょう!**

---

## 3.4 テスタブルなコードへのリファクタリング

定数とエラーメッセージをエクスポートして, テストから参照できるようにします:
...
- 定数を `export` してテストから参照可能に
- エラーメッセージをオブジェクトにまとめる
- `as const` で型を厳密に
...
-   it('141文字以上はエラー', () => {
+   it(`${MAX_MESSAGE_LENGTH}文字以上はエラー`, () => {
-     const text = 'a'.repeat(141);
+     const text = 'a'.repeat(MAX_MESSAGE_LENGTH + 1);
-     expect(validateMessage(text)).toContain('140');
-     expect(validateMessage(text)).toBe('140 文字以内で
...
## 3.5 validateFile 関数もリファクタリング

validateFile 関数も同様に定数とエラーメッセージをエクスポートします:
...
| 定数名 | 値 | 用途 |
|--------|-----|------|
| `MAX_SIZE` | 5MB | ファイルサイズ上限 |
| `MAX_FILENAME_LENGTH` | 100 | ファイル名長上限 |
| `ALLOWED_TYPES` | 配列 | 許可する MIME タイプ |

**メリット:**
- テストで `MAX_SIZE + 1` のように境界値を表現
- エラーメッセージの二重管理を防止
- 仕様変更時に1箇所だけ修正
...
**テストデータの意図:**
- `MAX_SIZE + 1` → 「上限を 1 MB 超過」
- `MAX_FILENAME_LENGTH + 1` → 「上限を 1 文字超過」
...
-   expect(validateFile(webpImage)).toBe('JPEG, PNG, GIF ファイルのみアップロード可能です');
+   expect(validateFile(webpImage)).toBe(FILE_ERROR_MESSAGES.INVALID_TYPE);
  });

  it('ファイルサイズ超過はエラー', () => {
-   expect(validateFile(overSizedImage)).toBe('ファイルサイズが大きすぎます（最大5MB）');
...
```
 ✓ src/utils/validator.test.ts (9 tests) 2ms
   ✓ validateMessage (5)
     ✓ 空文字の場合はエラーを返す 0ms
     ✓ 空白のみの場合もエラーを返す 0ms
     ✓ 正常な入力の場合は空文字を返す 0ms
     ...
     ✓ 141文字以上はエラー 0ms
   ✓ validateFile (4)
     ✓ 対象外の拡張子ファイルの場合はエラーを返す 0ms
     ✓ ファイルサイズが大きい場合はエラーを返す 0ms
     ✓ ファイル名が長すぎる場合はエラーを返す 0ms
     ✓ 正常な入力の場合は空文字を返す 0ms
...
**境界値テスト**が重要です:
- ちょうど `MAX_MESSAGE_LENGTH` → OK
- `MAX_MESSAGE_LENGTH + 1` → NG
...
**定数を使うメリット:**
- `140` より `MAX_MESSAGE_LENGTH` が意図明確
- 仕様変更時に自動で追従
...
つまり，**実装の詳細ではなく, ユーザーが見るものをテストする** ということ💁
...
- `vi.fn()` でモック関数を作成
- `await user.click()` でクリックイベント発火
- `toHaveBeenCalledTimes(1)` で呼び出し回数を確認

**なぜ `async/await` が必要?**

`userEvent` は実際のユーザー操作を再現するため非同期で動作します. `await` を忘れるとテストが正しく動作しません.
...
| マッチャー | 用途 |
|-----------|------|
| `toBeInTheDocument()` | DOM に存在する |
| `toBeDisabled()` | disabled 状態 |
| `toBeEnabled()` | enabled 状態 |
