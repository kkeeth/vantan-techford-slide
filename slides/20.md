...
## 3.4 validateFile 関数のテスト

ファイルバリデーション関数もテストしてみましょう. File オブジェクトをモックとして作成します:
...
**テストデータの作り方:**
- 無効な拡張子: `.webp`
- 大きすぎるファイル: `ArrayBuffer` で 6MB
- 長すぎるファイル名: `'a'.repeat(101)`
...
```ts
  ...
  it('ファイル名が長すぎる場合はエラーを返す', () => {
    expect(validateFile(longNameImage)).toBe(
      'ファイル名が長すぎます'
    );
  });
