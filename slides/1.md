...
- **前期（13回）**: TypeScriptの基礎からReactコンポーネント、状態管理、APIとの連携まで
...
- 前期終了時：基本的なToDoアプリが作成できる
- 講座終了時：本格的なWebアプリケーション（SNSアプリなど）を開発できる
...
現在のフロントエンドの開発環境のデファクトスタンダードが `TypeScript + React(Next.js)` だから
...
以下はJavaScriptとTypeScriptの比較をマークダウン形式のテーブルで表示します
