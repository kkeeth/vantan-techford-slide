...
### 4.3 PokemonCardコンポーネントの作成

データ表示用のPokemonCardコンポーネントを作成し、SearchForm内に配置してみましょう：
...
### 4.4 SearchForm内でPokemonCardを使用してデータ表示

取得したポケモンデータをstateで管理し、PokemonCardに渡して表示します：
...
-       console.log('名前:', pokemon.name);
-       console.log('画像:', pokemon.sprites?.front_default);
+       const result = await fetchPokemon(e.target[0].value);
+       setPokemon(result); // stateに保存
```
...
### 4.4 SearchForm内でPokemonCardを使用してデータ表示（続き）
