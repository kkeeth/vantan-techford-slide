...
PokemonCardコンポーネントで使用するPokemonオブジェクトの詳細な型定義を行います。
...
### 7.5 usePokemonSearch でのバリデーション統合
...
+     return { 
+       isValid: false, 
+       errorMessage: type === 'name' ? 'ポケモン名を入力してください' : 'ポケモン ID を入力してください'
+     };
+   }
+   
+   if (type === 'name') {
...
+       return { isValid: false, errorMessage: '1〜1010の数値を入力してください' };
+     }
+   }
+   
...
+     setError({ isError: true, message: validation.errorMessage });
+     return null;
+   }
+   
    setIsLoading(true);
...
+ const validateSearchInput = useCallback((query: string, type: SearchType) => {
+   return validateInput(query, type);
+ }, []);
...
- **関心の分離**: 検索関連のロジックが一箇所に集約
- **再利用性**: 他のコンポーネントでも同じ検索ロジックを使用可能
- **保守性**: バリデーションルールの変更が容易
- **テスト性**: フック単体でのテストが可能
- **型安全性**: ValidationResult 型で一貫した戻り値
...
```diff
// src/components/SearchForm.tsx
+ import { validatePokemonName, validatePokemonId } from '../utils/formValidation';
...
+     // リアルタイムバリデーション
+     if (value.trim()) {
+       const validation = searchType === 'name'
+         ? validatePokemonName(value)
+         : validatePokemonId(value);
