...
まずはJavaScriptで基本的なAPIクライアントを作成し、データ取得を確認します：
...
ここではまだ表示機能は追加せず、APIが正しく動作するかをコンソールで確認します💁
...
### 5.6 PokemonCardコンポーネントの型化

PokemonCardコンポーネントにTypeScript型を追加します：
...
適切な状態管理のために PokemonCard コンポーネントをApp.tsxに移動します💁App.tsxでデータフェッチを行い、各コンポーネントに適切にpropsを渡すように変更します：
...
SearchFormからデータ管理を削除し、検索機能のみに集中します：
