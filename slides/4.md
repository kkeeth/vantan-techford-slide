...
### 7.5 usePokemonSearch での fetchPokemon 呼び出し変更
...
- **引数の追加**: `fetchPokemon(query, type)` で SearchType を渡す
- **バリデーション自動実行**: API 層で入力値が自動検証される
- **統一されたエラー**: バリデーションエラーも ApiError として処理
- **シンプルな呼び出し**: usePokemonSearch は状態管理に集中
...
+ export const fetchPokemon = async (term: string): Promise<Pokemon> => {
    try {
      const response =
