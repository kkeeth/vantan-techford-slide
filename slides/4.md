...
+   const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
      e.preventDefault(); // デフォルトの送信動作を防止
+     const data = await fetchPokemon(e.target[0].value);
-     console.log("submit");
+     console.log(data);
    };
...
##### 現在は `SearchForm` コンポーネント内で直接データを取得しているため， `PokemonCard` コンポーネントにデータを渡さないと表示ができません．そのため，データの管理を親に任せることにします💁
...
+   const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
+     e.preventDefault();
+     const res = await fetchPokemon(e.target[0].value)
+     console.log(res);
+   }
```
...
##### 続いて，このアプリケーション内で使う型定義を行います💁コンポーネント内に閉じてもよいですが，コンポーネントを跨いで使うものもありますので，今回は別のファイルに集約させます．
...
export type Pokemon = {
  ...
  sprites: {           // ポケモンの画像
    other: {
      showdown: {
        front_default: string;  // 表面
        front_shiny: string;    // 表面（色違い）
        back_default: string;   // 裏面
        back_shiny: string;     // 裏面（色違い）
      };
    };
  };
  // 続く
...
詳しい仕様については[公式ドキュメント](https://pokeapi.co/docs/v2#pokemon)を見てみましょう🙋
...
型定義が終わりましたので，ここから実際に API をコールしてポケモンデータを取得してみましょう．
