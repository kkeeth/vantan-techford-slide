...
```tsx
  ...
  const handleAllRemoveTask = (tasksToRemove: Task[]) => {
    if (window.confirm(`完了済みタスクをすべて削除してもよいですか？`)) {
      const newTaskList = taskList.filter(
        (task) => !tasksToRemove.some(removeTask => removeTask.id === task.id)
      );
      setTaskList(newTaskList);
    }
  };
...
+   const [filter, setFilter] = useState<FilterType>('ALL');
  ...
  // 既存のhandleSubmit, handleTaskChange, handleRemoveTask は省略
...
+     <button
+       className="danger delete"
+       disabled={taskList.filter(task => task.isDone).length === 0}
+       onClick={() => handleAllRemoveTask(taskList.filter(task => task.isDone))}
+     >
+       delete all
+     </button>
