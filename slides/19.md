...
## 1.12 カスタムフック：useImageUpload の作成
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: colors.gradient }}>
      <Container maxWidth="md" sx={{ py: 4 }}>
        ...
        />
        {messages.map((message) => (
          <MessageItem key={message.id} message={message} onDeleteMessage={deleteMessage} />
        ))}
      </Container>
    </Box>
  );
...
+   message, onDeleteMessage, onEdit
  }: MessageItemProps) => {
```
